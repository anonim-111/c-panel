...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        #             neighborhood=Neighborhood.objects.filter(district_id=13, name=row[6].value).first()
        #         )

        neighborhoods = Neighborhood.objects.filter(doctors__isnull=True)
        print(neighborhoods.count())
        for neighborhood in neighborhoods:
            doctors = neighborhood.doctors.all().count()
            patients = neighborhood.patients.all().count()
            print(doctors, patients)
            if doctors == 0 and patients == 0:
                print(neighborhood)
